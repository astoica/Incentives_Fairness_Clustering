...
# this code creates a list, sensitive, that encodes the anonymized gender in the data; it is not used in this section
sensitive = []
for u in list_nodes:
    if (gender[u] == '1'):
        sensitive.append(1)
...
util_all_avg = np.mean([x for x in util_all.values()])
util_all_std = np.std([xx for xx in util_all.values()])
print("Elkind1 utility of random clustering: ",util_all_avg, util_all_std)
writer.writerow(['conductance mfhg'])
writer.writerow([conductance_random_avg])
writer.writerow([conductance_random_std])
writer.writerow(['mfhg utility'])
writer.writerow([util_all_avg])
writer.writerow([util_all_std])
myfile.close()